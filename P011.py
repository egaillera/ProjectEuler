'''
In the  grid below, four numbers along a diagonal line have been marked between *
...
grid[14] = [0,52,8,83,97,35,99,16,7,97,57,32,16,26,26,79,33,27,98,66]
...
# Transpose matrix
i = 0
tgrid = [[0 for x in range(20)] for y in range(20)] 
for row in zip(*grid):
    tgrid[i] = list(row)
    i += 1
...
for i in range(20):
	for j in range(17):
		a = reduce((lambda x, y: x * y), grid[i][j:j+4])
		b = reduce((lambda x, y: x * y), tgrid[i][j:j+4])
...
for i in range(17):
	...
	for j in range(l-3):
		a = reduce( (lambda x, y: x * y), [grid[x][x+i] for x in range(20-i)][j:j+4] )
		b = reduce( (lambda x, y: x * y), [grid[x+i][x] for x in range(20-i)][j:j+4] )
		c = reduce( (lambda x, y: x * y), [grid[x][19-x-i] for x in range(20-i)][j:j+4] )
		d = reduce( (lambda x, y: x * y), [grid[x+i][19-x] for x in range(20-i)][j:j+4] )
...
elapsed = time() - start                
print("Solution: {0} \t Time elapsed: {1}".format(max_product, elapsed))
